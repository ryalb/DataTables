...
(/** @lends <global> */function( $ ) {
  ...
  function _fnFilter( settings, input, force, regex, smart, caseInsensitive )
  {
    ...
    if ( input.length <= 0 ) {
      ...
      for ( i=display.length-1 ; i>=0 ; i-- ) {
        var value = settings.aoData[ display[i] ]._sFilterRow;
        value = value.replace(/(<([^>]+)>)/ig,'');
        if ( ! rpSearch.test( value ) ) {
          display.splice( i, 1 );
        }
      }
    }
  ...
  var setIcon = function (th) {
    var sortState = $(th).attr('aria-sort');
    var thisTable = $(th).closest('table').attr('id');

    if ( thisTable === undefined ) {
      $('#results .dataTable th').removeClass('sorting_asc sorting_desc');

      if (sortState === 'ascending') {

        $('#results .dataTable th').addClass('sorting');
